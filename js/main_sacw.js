...
class Main {
    ...
    constructor() {
        ...
        this._sessionId = window.localStorage.getItem('sessionId');
        ...
        if (!this._sessionId) {
            this._sessionId = Math.round(Math.random()*10e10).toString(36) + '-' + Date.now().toString(36);
    ...
    async _queryApi(action, params=null) {
        const rep = await window.fetch('https://qr-time.lubu.ch/php/api.php', {
            method: 'POST',
            credentials: 'include',
            headers: {
                'Content-Type': 'application/json'
            },
            body: JSON.stringify({sessionId: this._sessionId, action: action, params: params})
        });
    ...
    _showForm(data) {
        ...
        html += '<label for="fld_name">Vorname</label><input type="text" name="name" placeholder="Vorname" id="fld_name" value="' + name + '" required>';
        html += '</div>';

        html += '<div class="formEl textEl">';
        html += '<label for="fld_familyname">Nachname</label><input type="text" name="familyname" placeholder="Nachname" id="fld_familyname" value="' + familyname + '" required>';
        html += '</div>';

        html += '<div class="formEl textEl">';
        html += '<label for="fld_email">Email</label><input type="email" name="email" placeholder="Email-Adresse" id="fld_email" value="' + email + '" required>';
        html += '</div>';
